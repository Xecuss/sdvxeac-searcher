import { composeFilter, cateFilter, diffFilter, keywordFilter, packFilter } from "./filters";
import { Filter, ICompressedItem, ICompressedMusicItem, IMusicData, ISearchParams } from "./type";
...
export class EacSearcher {
    ...
    get packInfo() { return this.__raw.packInfo };
    get cateInfo() { return this.__raw.cateInfo };

    constructor(musicData: IMusicData, filters?: Filter[]) {
        this.musics = musicData.musics;
        this.__raw = musicData;
        musicData.packInfo.forEach(({ id, name }) => this.packIdMap.set(id, name));
        musicData.cateInfo.forEach(({ id, name }) => this.cateIdMap.set(id, name));
        if (filters) {
    ...
    public mapToLegacy(musics: ICompressedMusicItem[]): IMusicItem[] {
        return musics.map(music => ({
            ...
            category: music.category.map(cate => this.cateIdMap.get(cate) || ''),
